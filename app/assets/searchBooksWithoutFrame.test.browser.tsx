...
describe("SearchBooksWithoutFrame", () => {
  ...
  it("starts and resolves the initial populated query", async (t) => {
    ...
    let result = render(
      <SearchBooksWithoutFrame initialQuery="  dune  " />,
    );
    t.after(() => result.cleanup());
    ...
    assert.equal(
      initialRequest.url,
      routes.searchBooks.books.href(undefined, { q: "dune" }),
    );

    await result.act(async () => {
      initialRequest.resolve(
        new Response(JSON.stringify({ docs: [{ title: "Dune" }] })),
      );
      await settleSearch();
    });
    ...
    assert.deepEqual(
      [...result.container.querySelectorAll("li")].map(
  ...
  it("renders found, empty, invalid, and failed search outcomes", async (t) => {
    ...
    t.mock.method(window, "fetch", (input: RequestInfo) => {
      ...
      switch (query) {
        ...
        case "invalid":
          return Promise.resolve(
            new Response(
              JSON.stringify({ detail: [{ msg: "Invalid search query" }] }),
            ),
          );
        case "failed":
          return Promise.resolve(
    ...
    assert.match(
      result.container.textContent ?? "",
      /No books were found for this title/,
    );
    ...
    assert.match(
      result.container.textContent ?? "",
      /Could not fetch.+Invalid search query/,
    );
